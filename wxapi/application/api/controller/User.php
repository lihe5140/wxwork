...
class User extends Common
{
    ...
    private function checkRegisterUser()
    {
        ...
        // $userType = $this->checkUsername($this->datas['username']);

        //检测是否已经存在于数据库

        $this->checkExist($this->datas['username'], $userType, 0);
     ...
     * @return [type]        [description]
     */
    protected function checkUsernameExist($value)
    {
        $res = db('admin')->where('username', $value)->find();
        if(empty($res)){
